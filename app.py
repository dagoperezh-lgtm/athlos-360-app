# =============================================================================
# 🦅 ATHLOS 360 - V13.0 (PORTADA ULTRALIGERA + REPORTE V25)
# =============================================================================
...
# --- 2. GESTIÓN DE SESIÓN (ESTADO) ---
if 'club_activo' not in st.session_state:
    st.session_state['club_activo'] = None

# =============================================================================
# 🚪 FASE 1: PORTADA (SIN CARGA DE DATOS - INSTANTÁNEA)
# =============================================================================
    ...
    with c2:
        ...
        if club_sel == "TYM Triathlon":
            if st.button("INGRESAR AL DASHBOARD 🚀", type="primary", use_container_width=True):
                ...
                st.rerun() # Recarga inmediata para entrar
    
    # DETENEMOS EL SCRIPT AQUÍ PARA QUE NO CARGUE EXCEL TODAVÍA
    st.stop()

# =============================================================================
# 📊 FASE 2: MOTOR DE DATOS (SOLO SE EJECUTA SI ENTRASTE)
# =============================================================================
...
def get_df_cache(nombre_hoja):
    ...
    try:
        # Usamos context manager para cerrar el archivo rápido
...
# Sanitizadores V25 (Los que arreglaron los tiempos)
def clean_time(val):
    if pd.isna(val) or str(val).strip() in ['','-','nan','0','00:00:00','None']: return 0.0
    s = str(val).strip().split(' ')[-1] # Cortar fechas 1900
    try:
        if ':' in s:
...
with st.sidebar:
    ...
    # Cargar Base (Con Spinner visual)
    ...
    nombres.insert(0, " Selecciona tu nombre...") # Placeholder
    
    atleta = st.selectbox("Busca tu perfil:", nombres)

# PANTALLA DE BIENVENIDA AL CLUB (PORTADA 2)
if atleta == " Selecciona tu nombre...":
    ...
    with col_b:
        ...
        if os.path.exists("logo_athlos.png"):
            st.image("logo_athlos.png", use_container_width=True, caption="Powered by Athlos 360")

# REPORTE INDIVIDUAL (V25)
else:
    # Identificar última semana
    ...
    # Cargar Datos Completos (Lazy Load)
    data = {
    ...
    # RENDERIZADO (IDÉNTICO A V25 APROBADA)
    ...
    with c1:
        st.markdown(f"""<div class="card-box"><div class="stat-label">⏱️ Tiempo Total</div><div class="stat-value">{fmt_h_m(tv)}</div>
        <div class="comp-text">👥 Vs Eq: <span class="{ 'pos' if (tv-ta)>=0 else 'neg' }">{fmt_diff(tv-ta, True)}</span><br>
        📅 Vs Hist: <span class="{ 'pos' if (tv-th)>=0 else 'neg' }">{fmt_diff(tv-th, True)}</span></div></div>""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div class="card-box"><div class="stat-label">📏 Distancia</div><div class="stat-value">{dv:.1f} km</div>
        <div class="comp-text">👥 Vs Eq: <span class="{ 'pos' if (dv-da)>=0 else 'neg' }">{fmt_diff(dv-da)} km</span><br>
        📅 Vs Hist: <span class="{ 'pos' if (dv-dh)>=0 else 'neg' }">{fmt_diff(dv-dh)} km</span></div></div>""", unsafe_allow_html=True)
    with c3:
        st.markdown(f"""<div class="card-box"><div class="stat-label">⛰️ Altimetría</div><div class="stat-value">{av:.0f} m</div>
        <div class="comp-text">Acumulado Semanal</div></div>""", unsafe_allow_html=True)
    ...
    def draw_disc(tit, icon, cat, x_type):
        ...
        <table style="width:100%; font-size:14px;">
            <tr style="color:#666; border-bottom:1px solid #ddd;">
                <th style="text-align:left">Métrica</th><th style="text-align:left">Dato</th>
                <th style="text-align:left">Vs Equipo</th><th style="text-align:left">Vs Histórico</th>
            </tr>
            <tr><td><b>Tiempo</b></td><td>{fmt_h_m(t_v)}</td>
                <td class="{ 'pos' if (t_v-t_a)>=0 else 'neg' }">{fmt_diff(t_v-t_a, True)}</td>
                <td class="{ 'pos' if (t_v-t_h)>=0 else 'neg' }">{fmt_diff(t_v-t_h, True)}</td></tr>
            <tr><td><b>Distancia</b></td><td>{d_v:.1f} km</td>
                <td class="{ 'pos' if (d_v-d_a)>=0 else 'neg' }">{fmt_diff(d_v-d_a)}</td>
                <td class="{ 'pos' if (d_v-d_h)>=0 else 'neg' }">{fmt_diff(d_v-d_h)}</td></tr>
            <tr><td><b>{e_lbl}</b></td><td>{e_fmt}</td><td>-</td><td>-</td></tr>
